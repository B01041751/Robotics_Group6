...
class HazardWorldEnv(gym.Env):
    def __init__(self):
        ...
        # Load object positions from the layout written by generate_world.py.
        # Each key maps to one of the four navigation targets.
        ...
        try:
            ...
            _tun = _layout['tunnel']
            _tr  = _layout['tree']
            _fb1 = _layout['fireball1']
            _gas = _layout['gas']
        except Exception as _e:
            rospy.logwarn(f'Could not read layout.json ({_e}); using default target positions')
            _tun = {'x': 0.0,  'y': 7.0}
            _tr  = {'x': 8.0,  'y': 8.0}
            _fb1 = {'x': 5.0,  'y': 0.0}
            _gas = {'x': 2.0,  'y': -6.0}
        ...
        self.targets = {
            'TUNNEL_DEBRIS':  (_tun['x'], _tun['y']),
            'AVOID_TREE':     (_tr['x'],  _tr['y']),
            'FIREBALL_DODGE': (_fb1['x'], _fb1['y']),
            'GAS_ZONE':       (_gas['x'], _gas['y']),
        }
